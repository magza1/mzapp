...
class Transaction extends \Magento\Framework\Model\ResourceModel\Db\AbstractDb
{
     ...
     * @return multitype:
     */
    public function getReceivedCreditTransactionsByHour($from, $to='NOW()',$customerId=false){
        $connection = $this->getConnection();
        $select = $connection->select();
        $select->from(
            $this->getTable('ves_store_credit_transaction'),
        ...
        while (strtotime($datePointer) < $to){
            if(!isset($newData[$datePointer])){
                $result[$datePointer] =['time'=>$datePointer, 'credit'=>0];
            }else{
     ...
     * @return multitype:
     */
    public function getSpentCreditTransactionsByHour($from, $to='NOW()',$customerId=false){
        $connection = $this->getConnection();
        $select = $connection->select();
        $select->from(
            $this->getTable('ves_store_credit_transaction'),
     ...
     * @return multitype:
     */
    public function getReceivedCreditTransactionsByDay($from, $to='NOW()',$customerId=false){
        $connection = $this->getConnection();
        $select = $connection->select();
        $select->from(
            $this->getTable('ves_store_credit_transaction'),
    ...
    * @return multitype:
    */
    public function getSpentCreditTransactionsByDay($from, $to='NOW()',$customerId=false){
        $connection = $this->getConnection();
        $select = $connection->select();
        $select->from(
            $this->getTable('ves_store_credit_transaction'),
     ...
     * @return multitype:
     */
    public function getReceivedCreditTransactionsByMonth($from, $to='NOW()',$customerId=false){
        $connection = $this->getConnection();
        $select = $connection->select();
        $select->from(
            $this->getTable('ves_store_credit_transaction'),
    ...
    * @return multitype:
    */
    public function getSpentCreditTransactionsByMonth($from, $to='NOW()',$customerId=false){
        $connection = $this->getConnection();
        $select = $connection->select();
        $select->from(
            $this->getTable('ves_store_credit_transaction'),
