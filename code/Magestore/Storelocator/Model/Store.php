...
class Store extends \Magento\Framework\Model\AbstractModel
{
    ...
    public function import()
    {
        ...
        if($storeName == null || $storeName == ''){
            $storeName = substr(md5($data['store_name'] . rand(1,100)), 0, 20);
        }

        while(true){
            $stores = $this->getCollection()->addFieldToFilter('rewrite_request_path', $storeName);
            if(count($stores) > 0){
                    $temp_storeName = $storeName . '-' . substr(md5(rand(1,100)), rand(1,20), 3);
                    $storeName = $temp_storeName;
            }else{
                break;
            }
        }
