...
class Url extends \Magento\Framework\App\Helper\AbstractHelper
{
    ...
    public function characterSpecial($character) {
        ...
        $character = str_replace($input, $output, $character); //convert un-unicode char to unicode char
        $character = preg_replace('/[^A-Za-z0-9\-]+/i', '', $character); //remove all not allow char
        $character = preg_replace('/\-+/i', '-', $character); // replace multi - char to one - char
        $character = preg_replace('/(^\-+|\-+$)/i', '', $character); // remove all - char at first and end.
        return $character;
    }
}
