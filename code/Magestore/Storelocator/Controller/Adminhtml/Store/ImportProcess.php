...
use Magestore\Storelocator\Controller\Adminhtml\Store;

/**
 * Adminhtml Storelocator ProcessImport Action
 *
 * @category Magestore
...
class ImportProcess extends Store
{
    ...
    public function execute()
    {
        ...
        if (isset($_FILES['filecsv'])) {
			...
			$helperRegion = $this->_objectManager->create('Magestore\Storelocator\Helper\Region');
			...
			try {
                ...
                foreach ($data as $col => $row) {
					if ($col == 0) {
						...
						if(isset($storeData['state_id']) && $storeData['state_id'] == \Magestore\Storelocator\Helper\Region::STATE_ERROR){
						...
						if(isset($storeData['state_id']))
							$_state_id = $storeData['state_id'] > \Magestore\Storelocator\Helper\Region::STATE_ERROR;
    ...
    {
        return $this->_authorization->isAllowed('Magestore_Storelocator::storelocator');
    }
}
